...
data_dir = osp.normpath(osp.join(PROJ_ROOT, "datasets/lumi_piano_external_camera/data"))

cls_indexes = sorted(idx2class.keys())
cls_names = [idx2class[cls_idx] for cls_idx in cls_indexes]
lm_model_dir = osp.normpath(osp.join(PROJ_ROOT, "datasets/lumi_piano_external_camera/models"))
...
class XyzGen(object):
    ...
    def main(self):
        ...
        if track_num != "all":
            track_nums = [int(track_num)]
        else:
            track_start = 4
            track_end = 5
            track_nums = range(track_start, track_end+1)
        
        for track_idx in track_nums:
            data_root = self.data_root
            track_root = osp.join(data_root, f"track_{track_idx:02d}")
            ...
            if len(intr_dict) == 0:
                ...
                return
            K = np.array(intr_dict[list(intr_dict.keys())[0]]["cam_K"], dtype="float32").reshape(3, 3)
            ...
            for str_im_id in tqdm(gt_dict):
                ...
                for anno_i, anno in enumerate(gt_dict[str_im_id]):
                    ...
                    if mask.sum() == 0:  # NOTE: this should be ignored at training phase
                        print(
                            f"not visible, track {track_idx}, im {int_im_id} obj {idx2class[obj_id]} {obj_id}"
                        )
                        print(f"{save_path}")
                        xyz_info = {
                        ...
                        if VIS:
                            ...
                            mask_visib_path = osp.join(
                                data_root,
                                f"mask_visib/{int_im_id:05d}_{anno_i:05d}.png",
                            )
                            ...
                            raise RuntimeError(f"track {track_idx}, im {int_im_id}")
                    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--track", type=str, default="all", help="track num")
    ...
    setproctitle.setproctitle(f"gen_lumi_piano_by_track_{args.track}")
    xyz_gen = XyzGen(args.track)
    xyz_gen.main()
    T_end = time.perf_counter() - T_begin
    print("track", args.track, "total time: ", T_end)
