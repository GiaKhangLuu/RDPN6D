...
class SYN_LUMI_Piano_Dataset(object):
    ...
    def __init__(self, data_cfg):
        ...
        self.cat_ids = [cat_id for cat_id, obj_name in ref.syn_lumi_piano.id2obj.items() if obj_name in self.objs]
    ...
    def __call__(self):
        ...
        for ann_file, scene_root, xyz_root in zip(tqdm(self.ann_files), self.image_prefixes, self.xyz_prefixes):
            ...
            for im_id in tqdm(indices):
                ...
                track_id = int(rgb_path.split("/")[-3].split("synthetic_data")[-1])
                track_im_id = f"{track_id}/{int_im_id}"
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(self.models_root, f"obj_{ref.syn_lumi_piano.obj2id[obj_name]:06d}.ply"),
...
DATASET = dict(
    syn_lumi_piano_train=dict(
        name="syn_lumi_piano_train",
        dataset_root=osp.join(DATASETS_ROOT, "syn_lumi_piano_dataset"),
        models_root=osp.join(DATASETS_ROOT, "syn_lumi_piano_dataset/models"),
        objs=OBJECTS,  # selected objects
        ann_files=[
            osp.join(DATASETS_ROOT, f"syn_lumi_piano_dataset/data/synthetic_data{t}/image_set/syn_{OBJECTS[0]}_train.txt")
            for t in range(TRACK_START, TRACK_END + 1)
        ],
        image_prefixes=[
            osp.join(DATASETS_ROOT, f"syn_lumi_piano_dataset/data/synthetic_data{t}")
            for t in range(TRACK_START, TRACK_END + 1)
        ],
        xyz_prefixes=[
            osp.join(DATASETS_ROOT, f"syn_lumi_piano_dataset/data/synthetic_data{t}/xyz_crop")
            for t in range(TRACK_START, TRACK_END + 1)
        ],
        scale_to_meter=1,
        ...
        filter_scene=True,
        filter_invalid=True,
        ref_key="syn_lumi_piano",
    ),
    syn_lumi_piano_test=dict(
        name="syn_lumi_piano_test",
        dataset_root=osp.join(DATASETS_ROOT, "syn_lumi_piano_dataset"),
        models_root=osp.join(DATASETS_ROOT, "syn_lumi_piano_dataset/models"),
        objs=OBJECTS,  # selected objects
        ann_files=[
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in DATASET:
        ...
        used_cfg = data_cfg
    DatasetCatalog.register(name, SYN_LUMI_Piano_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="syn_lumi_piano",  # NOTE: for pvnet to determine module
