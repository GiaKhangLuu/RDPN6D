...
class LUMI_Piano_External_Camera_Dataset(object):
    ...
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg["dataset_root"]  # lumi_piano_dataset
        assert osp.exists(self.dataset_root), self.dataset_root
        self.models_root = data_cfg["models_root"]  # lumi_piano_dataset/models 
        self.scale_to_meter = data_cfg["scale_to_meter"]  # 1
        ...
        self.height = data_cfg["height"]  # 360
        self.width = data_cfg["width"]  # 640

        self.track_start = data_cfg["track_start"]
        self.track_end = data_cfg["track_end"]
        ...
        self.cat_ids = [cat_id for cat_id, obj_name in ref.lumi_piano_external_camera.id2obj.items() if obj_name in self.objs]
    ...
    def __call__(self):
        ...
        for track_idx in range(self.track_start, self.track_end + 1):

            for ann_file in tqdm(self.ann_files):
                ann_file = ann_file.format(f"{track_idx:02d}")
                with open(ann_file, "r") as f_ann:
                    indices = [line.strip("\r\n") for line in f_ann.readlines()]  # string ids

                scene_root = self.image_prefixes.format(f"{track_idx:02d}")
                xyz_root = self.xyz_prefixes.format(f"{track_idx:02d}")
                ...
                for im_id in tqdm(indices):
                    ...
                    depth_path = osp.join(scene_root, "depth/{:05d}.png".format(int_im_id))

                    track_im_id = f"{track_idx}/{int_im_id}"
                    ...
                    for anno_i, anno in enumerate(gt_dict[str_im_id]):
                        ...
                        t = np.array(anno["cam_t_m2c"], dtype="float32") / 1000.0  # mm to m
                        ...
                        mask_file = osp.join(scene_root, "mask/{:05d}_{:05d}.png".format(int_im_id, anno_i))
                        mask_visib_file = osp.join(scene_root, "mask_visib/{:05d}_{:05d}.png".format(int_im_id, anno_i))
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(self.models_root, f"obj_{ref.lumi_piano_external_camera.obj2id[obj_name]:06d}.ply"),
...
DATASET = dict(
    lumi_piano_external_camera_train=dict(
        name="lumi_piano_external_camera_train",
        dataset_root=osp.join(DATASETS_ROOT, "lumi_piano_external_camera"),
        models_root=osp.join(DATASETS_ROOT, "lumi_piano_external_camera/models"),
        objs=OBJECTS,  # selected objects
        ann_files=[
            osp.join(DATASETS_ROOT, "lumi_piano_external_camera/data/track_{2}/image_set/{0}_{1}.txt".format(_obj, "train", {}))
                for _obj in OBJECTS
        ],
        track_start=4,
        track_end=5,
        image_prefixes=osp.join(DATASETS_ROOT, "lumi_piano_external_camera/data/track_{}"),
        xyz_prefixes=osp.join(DATASETS_ROOT, "lumi_piano_external_camera/data/track_{}/xyz_crop"),
        scale_to_meter=1,
        ...
        filter_scene=True,
        filter_invalid=True,
        ref_key="lumi_piano_external_camera",
    ),
    lumi_piano_external_camera_test=dict(
        name="lumi_piano_external_camera_test",
        dataset_root=osp.join(DATASETS_ROOT, "lumi_piano_external_camera"),
        models_root=osp.join(DATASETS_ROOT, "lumi_piano_external_camera/models"),
        objs=OBJECTS,  # selected objects
        ann_files=[
            osp.join(DATASETS_ROOT, "lumi_piano_external_camera/data/track_{2}/image_set/{0}_{1}.txt".format(_obj, "test", {}))
                for _obj in OBJECTS
        ],
        track_start=4,
        track_end=5,
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in DATASET:
        used_cfg = DATASET[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, LUMI_Piano_External_Camera_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="lumi_piano_external_camera",  # NOTE: for pvnet to determine module
