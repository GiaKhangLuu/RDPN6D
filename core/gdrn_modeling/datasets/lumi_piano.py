...
class LUMI_Piano_Dataset(object):
    """lm splits."""
    ...
    def __init__(self, data_cfg):
        ...
        self.cat_ids = [cat_id for cat_id, obj_name in ref.lumi_piano.id2obj.items() if obj_name in self.objs]
    ...
    def __call__(self):  # LM_Dataset
        ...
        for ann_file, scene_root, xyz_root in zip(tqdm(self.ann_files), self.image_prefixes, self.xyz_prefixes):
            ...
            for im_id in tqdm(indices):
                ...
                track_id = int(rgb_path.split("/")[-3].split("_")[-1])
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(self.models_root, f"obj_{ref.lumi_piano.obj2id[obj_name]:06d}.ply"),
...
DATASET = dict(
    lumi_piano_train=dict(
        name="lumi_piano_train",
        dataset_root=osp.join(DATASETS_ROOT, "lumi_piano_dataset"),
        models_root=osp.join(DATASETS_ROOT, "lumi_piano_dataset/models"),
        objs=OBJECTS,  # selected objects
        ann_files=[
            osp.join(DATASETS_ROOT, f"lumi_piano_dataset/data/track_{t:02d}/image_set/{OBJECTS[0]}_train.txt")
            for t in range(TRACK_START, TRACK_END + 1) 
        ],
        image_prefixes=[
            osp.join(DATASETS_ROOT, f"lumi_piano_dataset/data/track_{t:02d}")
            for t in range(TRACK_START, TRACK_END + 1)
        ],
        xyz_prefixes=[
            osp.join(DATASETS_ROOT, f"lumi_piano_dataset/data/track_{t:02d}/xyz_crop/")
            for t in range(TRACK_START, TRACK_END + 1)
        ],
        scale_to_meter=1,
    ...
    lumi_piano_test=dict(
        name="lumi_piano_test",
        dataset_root=osp.join(DATASETS_ROOT, "lumi_piano_dataset"),
        models_root=osp.join(DATASETS_ROOT, "lumi_piano_dataset/models"),
        objs=OBJECTS,  # selected objects
        ann_files=[
            osp.join(DATASETS_ROOT, f"lumi_piano_dataset/data/track_{t:02d}/image_set/{OBJECTS[0]}_test.txt")
            for t in range(TRACK_START, TRACK_END + 1)
        ],
        image_prefixes=[
            osp.join(DATASETS_ROOT, f"lumi_piano_dataset/data/track_{t:02d}")
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in DATASET:
        ...
        used_cfg = data_cfg
    DatasetCatalog.register(name, LUMI_Piano_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="lumi_piano",  # NOTE: for pvnet to determine module
